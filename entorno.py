...
class Punto(object):
    def __init__(self, V):
        self.V = V
        self.artist = mpatches.Ellipse(np.array(self.V), 2, 2, alpha = 1, color = 'k')
...
class Poligonal(object):
    def __init__(self, V, alpha):
        ...
        isclosed = False
        if list(self.V[0]) == list(self.V[-1]):
            isclosed = True
        
        self.artist = mpatches.Polygon(V, closed = isclosed, fill = False, facecolor=None)
