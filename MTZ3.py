...
#            dc[c].start = dcs[c]
    ...
    for ind in range(len(tour)):
        ...
        for i in range(ind):
            clusters.append([tour[i]])
        
        while ind < len(tour):
            
            cluster = []
            cluster.append(tour[ind])
            
            while preprocessing3(datos, cluster):
                print("El cluster factible es: " + str(cluster))
                ...
                # Aqui consigo hasta el [0, 4, 3]
            
            cluster = cluster[0:-1]
            print("El cluster final es: " + str(cluster))
                        
            clusters.append(cluster)
        
        lista_de_soluciones.append(clusters)
        
    print(lista_de_soluciones)
    
    min_listas = min([len(lista) for lista in lista_de_soluciones])
    best_obj = 1e7
    best_list = []
    
    for listas in lista_de_soluciones:
        if len(listas) <= min_listas:
            
            print('Resolviendo el modelo para la lista: ' + str(listas))
            objetivo = MultiTargetPolygonal(datos, listas)
...
def preprocessing3(datos, lista):
    
    # Funcion que nos responde a la pregunta: es factible este subconjunto
    ...
    # Creacion del modelo
    MODEL = gp.Model('Feasibility Problem')
    ...
    dggt = MODEL.addVars(yggt_index, vtype = GRB.CONTINUOUS, lb = 0.0, name = 'dggt')
    difggt = MODEL.addVars(yggt_index, 2, vtype = GRB.CONTINUOUS, lb = 0.0, name = 'difggt')
    pggt = MODEL.addVars(yggt_index, vtype = GRB.CONTINUOUS, lb = 0.0, name = 'pggt')
    ...
    dt = MODEL.addVars(lista, vtype = GRB.CONTINUOUS, name = 'dt')
    ...
    MODEL.addConstr(ugt[lista[0]] >= 0.5)
    MODEL.addConstrs(mugt[g] >= 0.5 for g in lista)
    
    MODEL.addConstrs(yggt[lista[i], lista[i+1]] >= 0.5 for i in range(len(lista)-1))
    
    MODEL.addConstr(vgt[lista[-1]] >= 0.5)
    ...
    # restricciones = MODEL.addConstrs(gp.quicksum(yggt[g1, g2] for g1, g2 in permutations(set, 2)) <= len(set) - 1 for set in list(powerset(lista)))
    ...
    for g in lista:
        ...
        if type(comp) is Poligonal:
            ...
            BigM = comp.longitud
            MODEL.addConstr(pgt[g] >= SmallM * mugt[g])
            MODEL.addConstr(pgt[g] >= dt[g] - BigM* (1 - mugt[g]))
        if type(comp) is Punto:
            SmallM = 0
            BigM = 0
            MODEL.addConstr(pgt[g] >= SmallM * mugt[g])
    ...
    MODEL.addConstr((pgLt.sum('*') + pgt.sum('*') + gp.quicksum(pggt[g1, g2] for g1, g2 in pggt.keys()) + pgRt.sum('*'))/vD <= dLRt/vC)
    #
    ...
    # objective = dLRt + gp.quicksum(pgLt[index] for index in dgLt.keys()) + gp.quicksum(pgRt[index] for index in dgRt.keys()) + gp.quicksum(pgt[index] for index in pgt.keys()) + gp.quicksum(pggt[index] for index in pggt.keys())
    ...
    # MODEL.write('aver{0}.lp'.format(iter))
    iter += 1
...
def MultiTargetPolygonal(datos, listas):
    ...
    for lista, t in zip(listas, range(1, len(listas)+1)):
        ...
        if len(lista) == 1:
            MODEL.addConstr(mugt[lista[0], t] >= 0.5)
            MODEL.addConstr(ugt[lista[0], t] >= 0.5)
            MODEL.addConstr(vgt[lista[0], t] >= 0.5)
        ...
        if len(lista) > 1:
            ...
            for i in range(len(lista)-1):
                MODEL.addConstr(mugt[lista[i], t] >= 0.5)
                MODEL.addConstr(yggt[lista[i], lista[i+1], t] >= 0.5)
                
            MODEL.addConstr(mugt[lista[-1], t] >= 0.5)
            MODEL.addConstr(vgt[lista[-1], t] >= 0.5)
