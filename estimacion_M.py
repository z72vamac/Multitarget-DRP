...
def estima_BigM_local(comp1, comp2):
        ...
        if type(comp1) is Poligono or type(comp1) is Poligonal:
            ...
            if type(comp2) is Punto:
                maximo = max([np.linalg.norm(v-comp2.V) for v in comp1.V])               

        if type(comp1) is Punto:
            if type(comp2) is Punto:
                maximo = np.linalg.norm(np.array(comp1.V) - np.array(comp2.V))
            
            if type(comp2) is Poligonal:
                maximo = max([np.linalg.norm(comp1.V-w) for w in comp2.V])
...
def estima_SmallM_local(comp1, comp2):
        if type(comp1) is Poligono or type(comp1) is Poligonal:
            ...
            if type(comp2) is Punto:
                minimo = min([np.linalg.norm(v-comp2.V) for v in comp1.V])
                
        if type(comp1) is Punto:
            if type(comp2) is Punto:
                minimo = np.linalg.norm(np.array(comp1.V) - np.array(comp2.V))
            
            if type(comp2) is Poligonal:
                minimo = min([np.linalg.norm(comp1.V-w) for w in comp2.V])
