...
for key, it in zip(instancias.keys(), range(len(instancias.keys()))):
    ...
    if size >= 5 and size <= 15: #and iter in [244, 250, 252, 267]: #and iter >= 127 + 63 + 62: #and iter >= 59: #and iter >= 45 + 19: #and iter >= 188 + 13:
        ...
        print()
        print('--------------------------------------------')
        print('MultiTargetPoint: Instance: {a} - Size: {b} - Capacity: {c}'.format(a = instance, b = size, c = capacity))
        print('--------------------------------------------')
        print()
            
        solution = MultiTargetPolygonal(datos, 2)
        ...
        dataframe.to_csv('results_polygonal_gurobi_gettingtime2.csv', header = True, mode = 'w')
        
        # avisador(iter, 'point with initialization')
    iter += 1
